...
export const Button: React.FC<ButtonProps> = ({
  ...
  const getContainerStyle = (): ViewStyle => {
    ...
    if (disabled) {
      ...
      // Disable pointer events style for disabled button
      if (Platform.OS === 'web') {
        (baseStyle as any).cursor = 'not-allowed';
      }
    }
  ...
  // Web optimized hover/transition could be added here if we switched to Pressable
  // For now, adhering to TouchableOpacity with cursor pointer
      ...
      return (
          <TouchableOpacity
            style={[
                styles.container, 
                getContainerStyle(), 
                { padding: 0, overflow: 'hidden', borderWidth: 0 }, 
                style,
                !disabled && webStyles.pointer
            ]}
...
const styles = StyleSheet.create({
  container: {
    ...
    justifyContent: 'center',
    borderRadius: 16,
    // Web transition for smooth hover effect
    ...Platform.select({
        web: {
